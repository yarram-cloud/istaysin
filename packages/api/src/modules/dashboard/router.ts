...
import { prisma, getLocalDate } from '../../config/database';
  ...
  try {
    ...
    const [
      ...
      prisma.booking.count({
        where: { tenantId, checkInDate: new Date(today), status: { in: ['confirmed', 'checked_in'] } },
      }),
      prisma.booking.count({
        where: { tenantId, checkOutDate: new Date(today), status: 'checked_in' },
      }),
      prisma.booking.count({
