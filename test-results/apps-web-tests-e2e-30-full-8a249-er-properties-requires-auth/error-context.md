...
- Name: apps\web\tests\e2e\30-full-platform.spec.ts >> 03 — Tenants Module >> TENANT-02: Listing user properties requires auth
- Location: apps\web\tests\e2e\30-full-platform.spec.ts:72:7
...
> 74  |     expect(res.status()).toBe(401);
  ...
  172 | 
  173 | // ═══════════════════════════════════════════════════
  174 | // 9. PUBLIC MODULE (no auth required)
```