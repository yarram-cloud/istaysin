...
import { Key, ArrowLeft, Loader2, Save, Trash2 } from 'lucide-react';
import { toast } from 'sonner';
import { useRouter } from 'next/navigation';

export default function EditInventoryRoomPage({ params }: { params: { id: string } }) {
  ...
  const fetchDependencies = useCallback(async () => {
    try {
      ...
      if (rRes.success) {
        const room = rRes.data?.find((r: any) => r.id === params.id);
        if (room) {
          setFormData({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      await roomsApi.updateRoom(params.id, formData);
